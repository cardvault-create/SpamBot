...
API_ID = int(os.getenv("API_ID", "e7747bfa480eb76256f96976b9dccabc"))
API_HASH = os.getenv("API_HASH", "37864401")
...
class PremiumGroupSpamBot:
    ...
    def load_data(self):
        ...
        try:
            with open(GROUPS_FILE, 'r') as f: saved_groups = json.load(f)
        except: saved_groups = {}
        try:
            with open(ADMIN_FILE, 'r') as f: admin_list = set(json.load(f))
        except: admin_list = set()
        admin_list.add(self.owner_id)
        try:
            with open(CONFIG_FILE, 'r') as f: auto_delete_enabled = json.load(f).get("auto_delete", True)
        except: auto_delete_enabled = True
        try:
            with open(ADMIN_GROUPS_FILE, 'r') as f: admin_groups = json.load(f)
        except: admin_groups = {}
        try:
            with open(AUTO_DELETE_FILE, 'r') as f: auto_delete_tasks = json.load(f)
        except: auto_delete_tasks = {}
    
    def save_groups(self): json.dump(saved_groups, open(GROUPS_FILE, 'w'))
    def save_admins(self): json.dump(list(admin_list), open(ADMIN_FILE, 'w'))
    def save_config(self): json.dump({"auto_delete": auto_delete_enabled}, open(CONFIG_FILE, 'w'))
    def save_admin_groups(self): json.dump(admin_groups, open(ADMIN_GROUPS_FILE, 'w'))
    def save_auto_delete_tasks(self): json.dump(auto_delete_tasks, open(AUTO_DELETE_FILE, 'w'))
    
    def is_authorized(self, user_id): return user_id == self.owner_id or user_id in admin_list
    def get_user_groups(self, user_id): return saved_groups if user_id == self.owner_id else admin_groups.get(str(user_id), {})
    def save_user_groups(self, user_id, groups):
        if user_id == self.owner_id:
            global saved_groups
            saved_groups = groups; self.save_groups()
        else:
            admin_groups[str(user_id)] = groups; self.save_admin_groups()
    
    def bi(self, text): return f"***{text}***"
    ...
    def get_style_function(self, style):
        styles = {"bold": self.style_bold, "italic": self.style_italic, "bold_italic": self.style_bold_italic, "mono": self.style_mono, "sans": self.style_sans, "sans_bold": self.style_sans_bold, "sans_italic": self.style_sans_italic, "sans_bi": self.style_sans_bi, "serif_bold": self.style_serif_bold}
...
"""
    ...
    async def get_chat_info(self, context, chat_id):
        try:
            ...
            try: count = await context.bot.get_chat_member_count(chat_id)
            except: count = 0
            try: admin_count = len(await context.bot.get_chat_administrators(chat_id))
            ...
            return {"title": title, "type": ctype, "members": count, "admins": admin_count, "desc": desc, "online": online}
        except:
    ...
    async def schedule_auto_delete_group(self, context, chat_id, hours=24):
        ...
        auto_delete_tasks[task_key] = {"enabled": True, "hours": hours, "added_at": datetime.datetime.now().isoformat()}
...
"""
    ...
    async def getid_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if chat_type in ["group", "supergroup"]:
            saved_groups[str(chat_id)] = {"name": chat_title, "type": chat_type, "id": str(chat_id), "members": str(members), "admins": str(admins), "online": str(online)}
            self.save_groups()
    ...
    async def handle_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if data == "stop_spam":
            if user_id in active_spam_tasks:
                ...
                try: await query.edit_message_text(f"{self.bi('🛑 𝗦𝗣𝗔𝗠 𝗦𝗧𝗢𝗣𝗣𝗘𝗗 🛑')}", parse_mode=ParseMode.MARKDOWN)
                except: pass
            else:
        ...
        if data == "delete_last_spam":
            if user_id in last_spam_messages:
                ...
                await query.edit_message_text(f"{self.bi('🗑 𝗗𝗲𝗹𝗲𝘁𝗶𝗻𝗴 𝗠𝗲𝘀𝘀𝗮𝗴𝗲𝘀...')}", parse_mode=ParseMode.MARKDOWN)
                deleted = 0
                ...
                await query.edit_message_text(
                    f"{self.bi('✅ 𝗗𝗘𝗟𝗘𝗧𝗘𝗗: ' + str(deleted) + '/' + str(len(sd.get('message_ids',[]))))}\n{self.bi('📌 ' + sd.get('group_name',''))}",
                    reply_markup=InlineKeyboardMarkup(kb), parse_mode=ParseMode.MARKDOWN
                )
        ...
        if data == "get_id_info":
            kb = [[InlineKeyboardButton("🔙 𝗕𝗔𝗖𝗞 𝗧𝗢 𝗠𝗘𝗡𝗨", callback_data="main_menu")]]
            await query.edit_message_text(
                f"""{self.bi('🆔 𝗛𝗢𝗪 𝗧𝗢 𝗚𝗘𝗧 𝗚𝗥𝗢𝗨𝗣 𝗜𝗗 🆔')}
{self.bi('━━━━━━━━━━━━━━━━━━')}
{self.bi('📌 𝗦𝘁𝗲𝗽 𝟭: 𝗔𝗱𝗱 𝗕𝗼𝘁 𝗔𝘀 𝗔𝗗𝗠𝗜𝗡')}
{self.bi('📌 𝗦𝘁𝗲𝗽 𝟮: 𝗦𝗲𝗻𝗱 /𝗚𝗲𝘁𝗜𝗱 𝗜𝗻 𝗚𝗿𝗼𝘂𝗽')}
{self.bi('📌 𝗦𝘁𝗲𝗽 𝟯: 𝗕𝗼𝘁 𝗦𝗵𝗼𝘄𝘀 𝗙𝘂𝗹𝗹 𝗜𝗻𝗳𝗼')}
{self.bi('✅ 𝗚𝗿𝗼𝘂𝗽 𝗔𝘂𝘁𝗼-𝗦𝗮𝘃𝗲𝗱')}""",
                reply_markup=InlineKeyboardMarkup(kb), parse_mode=ParseMode.MARKDOWN
            )
            return
        if data == "saved_groups_menu":
            ...
            if not user_groups:
                kb = [[InlineKeyboardButton("🔙 𝗕𝗔𝗖𝗞 𝗧𝗢 𝗠𝗘𝗡𝗨", callback_data="main_menu")]]
                await query.edit_message_text(f"{self.bi('📋 𝗡𝗼 𝗚𝗿𝗼𝘂𝗽𝘀 𝗦𝗮𝘃𝗲𝗱')}\n{self.bi('💡 𝗦𝗲𝗻𝗱 /𝗚𝗲𝘁𝗜𝗱 𝗜𝗻 𝗚𝗿𝗼𝘂𝗽')}", reply_markup=InlineKeyboardMarkup(kb), parse_mode=ParseMode.MARKDOWN)
                return
            keyboard = []
            ...
            await query.edit_message_text(
                f"{self.bi('📋 𝗦𝗔𝗩𝗘𝗗 𝗚𝗥𝗢𝗨𝗣𝗦 (' + str(len(user_groups)) + ')')}\n{self.bi('👇 𝗦𝗘𝗟𝗘𝗖𝗧 𝗔 𝗚𝗥𝗢𝗨𝗣')}",
        ...
        if data == "clear_groups":
            ...
            kb = [[InlineKeyboardButton("🔙 𝗕𝗔𝗖𝗞 𝗧𝗢 𝗠𝗘𝗡𝗨", callback_data="main_menu")]]
            await query.edit_message_text(f"{self.bi('🗑 𝗚𝗥𝗢𝗨𝗣𝗦 𝗖𝗟𝗘𝗔𝗥𝗘𝗗')}\n{self.bi('✅ 𝗔𝗹𝗹 𝗦𝗮𝘃𝗲𝗱 𝗚𝗿𝗼𝘂𝗽𝘀 𝗗𝗲𝗹𝗲𝘁𝗲𝗱')}", reply_markup=InlineKeyboardMarkup(kb), parse_mode=ParseMode.MARKDOWN)
            return
        if data == "add_group_manual":
            user_states[user_id] = {"step": "waiting_for_group_manual"}
            kb = [[InlineKeyboardButton("🔙 𝗕𝗔𝗖𝗞 𝗧𝗢 𝗠𝗘𝗡𝗨", callback_data="main_menu")]]
            await query.edit_message_text(
                f"""{self.bi('➕ 𝗔𝗗𝗗 𝗚𝗥𝗢𝗨𝗣 𝗠𝗔𝗡𝗨𝗔𝗟𝗟𝗬 ➕')}
{self.bi('━━━━━━━━━━━━━━━━━━')}
...
{self.bi('👇 𝗦𝗲𝗹𝗲𝗰𝘁 𝗢𝗽𝘁𝗶𝗼𝗻 👇')}""",
        ...
        if data == "enable_auto_24":
            auto_delete_enabled = True; self.save_config()
            await query.answer("✅ 𝟮𝟰𝗛 𝗔𝘂𝘁𝗼 𝗗𝗲𝗹𝗲𝘁𝗲 𝗘𝗻𝗮𝗯𝗹𝗲𝗱!", show_alert=True)
            await self.show_main_menu(query); return
        if data == "disable_auto_del":
            auto_delete_enabled = False; self.save_config()
        ...
        if data == "delete_all_group":
            ...
            if not user_groups:
                ...
                await query.edit_message_text(f"{self.bi('📋 𝗡𝗼 𝗚𝗿𝗼𝘂𝗽𝘀 𝗦𝗮𝘃𝗲𝗱')}", reply_markup=InlineKeyboardMarkup(kb), parse_mode=ParseMode.MARKDOWN)
                return
            keyboard = []
            for gid, ginfo in list(user_groups.items())[:20]:
                name = ginfo.get("name", gid)[:30]
...
{self.bi('━━━━━━━━━━━━━━━━━━')}
{self.bi('⚠ 𝗕𝗼𝘁 𝗺𝘂𝘀𝘁 𝗯𝗲 𝗔𝗗𝗠𝗜𝗡 𝘄𝗶𝘁𝗵 𝗗𝗘𝗟𝗘𝗧𝗘 𝗽𝗲𝗿𝗺𝗶𝘀𝘀𝗶𝗼𝗻')}
...
{self.bi('📌 𝗚𝗿𝗼𝘂𝗽: ' + gname)}
{self.bi('⏳ 𝗦𝗰𝗮𝗻𝗻𝗶𝗻𝗴 𝗹𝗮𝘀𝘁 𝟮𝟬𝟬 𝗺𝗲𝘀𝘀𝗮𝗴𝗲𝘀...')}""",
                parse_mode=ParseMode.MARKDOWN
            )
            
            # Check bot permissions
            is_admin = False; can_delete = False; error_note = None
            try:
            ...
            deleted = 0; failed = 0
            if not is_admin: error_note = "❌ Bot is NOT ADMIN in this group"
            elif not can_delete: error_note = "❌ Bot does NOT have DELETE permission"
            else:
                try:
                    async for msg in context.bot.get_chat_history(chat_id=chat_id, limit=200):
                        if msg.from_user and msg.from_user.id == context.bot.id:
                            try:
                                ...
                                deleted += 1; await asyncio.sleep(0.1)
                            except: failed += 1
                except: pass
            
            kb = [[InlineKeyboardButton("🔙 𝗕𝗔𝗖𝗞 𝗧𝗢 𝗣𝗔𝗡𝗘𝗟", callback_data="owner_panel")]]
            if deleted > 0:
                msg_text = f"{self.bi('✅ 𝗗𝗘𝗟𝗘𝗧𝗘𝗗: ' + str(deleted) + ' 𝗠𝗲𝘀𝘀𝗮𝗴𝗲𝘀')}\n{self.bi('📌 ' + gname)}"
            else:
                msg_text = f"{self.bi('❌ 𝗡𝗢 𝗠𝗘𝗦𝗦𝗔𝗚𝗘𝗦 𝗗𝗘𝗟𝗘𝗧𝗘𝗗 ❌')}\n{self.bi('📌 ' + gname)}\n"
                if error_note: msg_text += f"{self.bi(error_note)}\n"
                msg_text += f"""{self.bi('━━━━━━━━━━━━━━')}
{self.bi('💡 𝗦𝗢𝗟𝗨𝗧𝗜𝗢𝗡:')}
{self.bi('𝟭️ 𝗠𝗮𝗸𝗲 𝗯𝗼𝘁 𝗔𝗗𝗠𝗜𝗡')}
{self.bi('𝟮️ 𝗘𝗻𝗮𝗯𝗹𝗲 𝗗𝗘𝗟𝗘𝗧𝗘 𝗽𝗲𝗿𝗺𝗶𝘀𝘀𝗶𝗼𝗻')}
{self.bi('𝟯️ 𝗧𝗿𝘆 𝗮𝗴𝗮𝗶𝗻')}"""
            await query.edit_message_text(msg_text, reply_markup=InlineKeyboardMarkup(kb), parse_mode=ParseMode.MARKDOWN)
            return
        if data == "manage_admins":
            if user_id != self.owner_id:
                await query.answer("🔐 𝗢𝗻𝗹𝘆 𝗢𝘄𝗻𝗲𝗿!", show_alert=True); return
        ...
        if data.startswith("style_"):
            ...
            style_names = {"text":"💬 𝗡𝗢𝗥𝗠𝗔𝗟","bold":"𝗕 𝗕𝗢𝗟𝗗","italic":"𝑰 𝗜𝗧𝗔𝗟𝗜𝗖","bold_italic":"𝑩𝑰 𝗕𝗢𝗟𝗗+𝗜𝗧𝗔𝗟𝗜𝗖","mono":"𝙼 𝗠𝗢𝗡𝗢","sans":"𝖲 𝗦𝗔𝗡𝗦","sans_bold":"𝗦𝗕 𝗦𝗔𝗡𝗦 𝗕𝗢𝗟𝗗","sans_italic":"𝘚𝘐 𝘚𝘈𝘕𝘚 𝘐𝘛𝘈𝘓𝘐𝘊","sans_bi":"𝙎𝘽𝙄 𝙎𝘼𝙉𝙎 𝘽𝙄","serif_bold":"𝐒𝐁 𝐒𝐄𝐑𝐈𝐅 𝐁𝐎𝐋𝐃"}
        ...
        if data.startswith("media_"):
            ...
            user_states[user_id]["step"] = "waiting_for_content"
            kb = [[InlineKeyboardButton("🔙 𝗕𝗔𝗖𝗞 𝗧𝗢 𝗦𝗧𝗬𝗟𝗘𝗦", callback_data="show_styles")]]
            await query.edit_message_text(f"{self.bi('📤 𝗦𝗲𝗻𝗱 𝘆𝗼𝘂𝗿 𝗺𝗲𝗱𝗶𝗮:')}", reply_markup=InlineKeyboardMarkup(kb), parse_mode=ParseMode.MARKDOWN)
            return
        if data == "show_styles":
            await self.show_style_menu(query, user_id); return
        if data == "show_count":
            user_states[user_id]["step"] = "waiting_for_count"
            kb = [[InlineKeyboardButton("🔙 𝗕𝗔𝗖𝗞 𝗧𝗢 𝗦𝗧𝗬𝗟𝗘𝗦", callback_data="show_styles")]]
            await query.edit_message_text(
                f"""{self.bi('🔢 𝗛𝗢𝗪 𝗠𝗔𝗡𝗬 𝗧𝗜𝗠𝗘𝗦? (𝟭-𝟭𝟬𝟬) 🔢')}
{self.bi('━━━━━━━━━━━━━━━━━━')}
...
{self.bi('💡 𝗥𝗲𝗰𝗼𝗺𝗺𝗲𝗻𝗱𝗲𝗱: 𝟭𝟬 𝗙𝗼𝗿 𝗧𝗲𝘀𝘁')}""",
        ...
        if data.startswith("speed_"):
            speed_map = {"speed_ultra": 0.1, "speed_fast": 0.3, "speed_normal": 0.5, "speed_slow": 1.5}
            user_states[user_id]["delay"] = speed_map.get(data, 0.5)
            await self.execute_spam(query, user_id, context); return
        if data == "resend_same":
    ...
    async def show_style_menu(self, query, user_id):
        ...
        await query.edit_message_text(
            f"""{self.bi('🎯 𝗦𝗘𝗟𝗘𝗖𝗧 𝗦𝗧𝗬𝗟𝗘 𝗢𝗥 𝗠𝗘𝗗𝗜𝗔 🎯')}
{self.bi('━━━━━━━━━━━━━━━━━━')}
{self.bi('📌 𝗚𝗿𝗼𝘂𝗽: ' + gname)}
{self.bi('━━━━━━━━━━━━━━━━━━')}
...
{self.bi('👇 𝗖𝗵𝗼𝗼𝘀𝗲 𝗙𝗿𝗼𝗺 𝟭𝟬 𝗦𝘁𝘆𝗹𝗲𝘀 𝗢𝗿 𝟳 𝗠𝗲𝗱𝗶𝗮 👇')}""",
    ...
    async def message_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # ✅ GROUP MESSAGES COMPLETELY IGNORE
        if chat_type in ["group", "supergroup"]:
            return
        ...
        state = user_states[user_id]; step = state.get("step")
        
        if step == "waiting_for_admin_id":
            if user_id != self.owner_id: return
            try:
                tid = int(update.message.text.strip())
                if tid in admin_list: admin_list.remove(tid); self.save_admins(); msg = f"✅ 𝗥𝗲𝗺𝗼𝘃𝗲𝗱: {tid}"
                else: admin_list.add(tid); self.save_admins(); msg = f"✅ 𝗔𝗱𝗱𝗲𝗱: {tid}"
                await update.message.reply_text(f"{self.bi(msg)}", parse_mode=ParseMode.MARKDOWN)
            except: await update.message.reply_text(f"{self.bi('❌ 𝗜𝗻𝘃𝗮𝗹𝗶𝗱')}", parse_mode=ParseMode.MARKDOWN)
            return
        ...
        if step in ["waiting_for_group_manual", "waiting_for_group"]:
            text = update.message.text; chat_id = None
            if re.match(r'^-?\d{10,}$', text.strip()): chat_id = int(text.strip())
            elif re.search(r'@([a-zA-Z0-9_]+)', text): chat_id = f"@{re.search(r'@([a-zA-Z0-9_]+)', text).group(1)}"
            elif re.match(r'^[a-zA-Z0-9_]{4,}$', text.strip()): chat_id = f"@{text.strip()}"
            if chat_id:
                info = await self.get_chat_info(context, chat_id); gname = info["title"]
                user_groups = self.get_user_groups(user_id)
                user_groups[str(chat_id)] = {"name": gname, "id": str(chat_id), "members": str(info["members"]), "online": str(info["online"])}
                ...
                if auto_delete_enabled: asyncio.create_task(self.schedule_auto_delete_group(context, chat_id, 24))
                keyboard = [
                    [InlineKeyboardButton("💬 𝗡𝗢𝗥𝗠𝗔𝗟", callback_data="style_text"), InlineKeyboardButton("𝗕 𝗕𝗢𝗟𝗗", callback_data="style_bold")],
                    [InlineKeyboardButton("𝑰 𝗜𝗧𝗔𝗟𝗜𝗖", callback_data="style_italic"), InlineKeyboardButton("𝑩𝑰 𝗕+𝗜", callback_data="style_bold_italic")],
                    [InlineKeyboardButton("🎯 𝗦𝗧𝗜𝗖𝗞𝗘𝗥", callback_data="media_sticker"), InlineKeyboardButton("🖼 𝗣𝗛𝗢𝗧𝗢", callback_data="media_photo")],
                    [InlineKeyboardButton("🔙 𝗕𝗔𝗖𝗞", callback_data="main_menu")]
                ]
                await update.message.reply_text(
                    f"""{self.bi('✅ 𝗚𝗥𝗢𝗨𝗣 𝗙𝗢𝗨𝗡𝗗 & 𝗦𝗔𝗩𝗘𝗗 ✅')}
{self.bi('━━━━━━━━━━━━━━━━━━')}
{self.bi('📌 𝗡𝗮𝗺𝗲: ' + gname)}
{self.bi('🆔 𝗜𝗗: ' + str(chat_id))}
...
{self.bi('📌 𝗦𝗲𝗻𝗱 𝗔 𝗡𝘂𝗺𝗯𝗲𝗿 𝗳𝗿𝗼𝗺 𝟭 𝘁𝗼 𝟭𝟬𝟬')}""",
        ...
        if step == "waiting_for_count":
            try:
                count = int(update.message.text); count = max(1, min(count, 100))
                user_states[user_id]["count"] = count; user_states[user_id]["step"] = "waiting_for_speed"
                keyboard = [
                    ...
                    [InlineKeyboardButton("🐢 𝗡𝗢𝗥𝗠𝗔𝗟 (𝟬.𝟱𝘀)", callback_data="speed_normal"),
                     InlineKeyboardButton("🦥 𝗦𝗟𝗢𝗪 (𝟭.𝟱𝘀)", callback_data="speed_slow")],
                    [InlineKeyboardButton("🔙 𝗕𝗔𝗖𝗞 𝗧𝗢 𝗖𝗢𝗨𝗡𝗧", callback_data="show_count")]
                ]
                await update.message.reply_text(
                    f"""{self.bi('⚡ 𝗖𝗛𝗢𝗢𝗦𝗘 𝗦𝗣𝗘𝗘𝗗 ⚡')}
{self.bi('━━━━━━━━━━━━━━━━━━')}
...
{self.bi('👇 𝗦𝗲𝗹𝗲𝗰𝘁 𝗦𝗽𝗲𝗲𝗱 𝗕𝗲𝗹𝗼𝘄 👇')}""",
                ...
                kb = [[InlineKeyboardButton("🔙 𝗕𝗔𝗖𝗞 𝗧𝗢 𝗦𝗧𝗬𝗟𝗘𝗦", callback_data="show_styles")]]
                await update.message.reply_text(f"{self.bi('❌ 𝗜𝗡𝗩𝗔𝗟𝗜𝗗 𝗡𝗨𝗠𝗕𝗘𝗥 ❌')}\n{self.bi('𝗦𝗲𝗻𝗱 𝟭 𝘁𝗼 𝟭𝟬𝟬')}", reply_markup=InlineKeyboardMarkup(kb), parse_mode=ParseMode.MARKDOWN)
            return
    
    def parse_chat_id(self, text):
        if not text: return None
        text = text.strip()
        if re.match(r'^-?\d{10,}$', text): return int(text)
        match = re.search(r'@([a-zA-Z0-9_]+)', text)
        if match: return f"@{match.group(1)}"
        match = re.search(r'(?:https?://)?t\.me/([a-zA-Z0-9_]+)', text)
        if match: return f"@{match.group(1)}"
        if re.match(r'^[a-zA-Z0-9_]{4,}$', text): return f"@{text}"
        return None
    
    async def execute_spam(self, query, user_id, context):
        data = user_states.get(user_id, {})
        chat_id = data.get("chat_id", ""); gname = data.get("group_name", str(chat_id))
        count = data.get("count", 1); delay = data.get("delay", 0.5)
        msg_type = data.get("msg_type", "text"); content = data.get("content")
        ...
        status_msg = await context.bot.send_message(
            ...
            text=f"""{self.bi('🔥 𝗦𝗣𝗔𝗠 𝗔𝗧𝗧𝗔𝗖𝗞 𝗦𝗧𝗔𝗥𝗧𝗘𝗗 🔥')}
{self.bi('━━━━━━━━━━━━━━━━━━')}
{self.bi('📌 𝗚𝗿𝗼𝘂𝗽: ' + gname)}
...
{self.bi('🚀 𝗦𝗘𝗡𝗗𝗜𝗡𝗚 𝗠𝗘𝗦𝗦𝗔𝗚𝗘𝗦 𝗡𝗢𝗪... 🚀')}""",
        ...
        success = 0; failed = 0; consecutive_fails = 0; sent_message_ids = []
        text_types = ["text", "bold", "italic", "bold_italic", "mono", "sans", "sans_bold", "sans_italic", "sans_bi", "serif_bold"]
        style_func = self.get_style_function(msg_type) if msg_type in text_types and msg_type != "text" else None
        
        for i in range(count):
            if not active_spam_tasks.get(user_id, False):
                try: await status_msg.edit_text(f"{self.bi('🛑 𝗦𝗧𝗢𝗣𝗣𝗘𝗗 🛑')}\n{self.bi('✅ ' + str(success) + '/' + str(count))}", parse_mode=ParseMode.MARKDOWN)
                except: pass
                break
            try:
                sent_msg = None
                if msg_type in text_types:
                    ...
                    if style_func: txt = style_func(txt)
                ...
                elif msg_type == "voice" and content and content.voice:
                    sent_msg = await context.bot.send_voice(chat_id=chat_id, voice=content.voice.file_id, caption=content.caption or "")
                elif msg_type == "video_note" and content and content.video_note:
                    sent_msg = await context.bot.send_video_note(chat_id=chat_id, video_note=content.video_note.file_id)
                
                if sent_msg:
                    sent_message_ids.append(sent_msg.message_id); success += 1; consecutive_fails = 0
                    if auto_delete_enabled: asyncio.create_task(self.auto_delete_msg_after(context, chat_id, sent_msg.message_id, 24))
                
                if success % 10 == 0:
                    try: await status_msg.edit_text(f"{self.bi('⚡ 𝗦𝗣𝗔𝗠𝗠𝗜𝗡𝗚... ⚡')}\n{self.bi('✅ ' + str(success) + '/' + str(count))}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🛑 𝗦𝗧𝗢𝗣 🛑", callback_data="stop_spam")]]), parse_mode=ParseMode.MARKDOWN)
                    except: pass
                
                await asyncio.sleep(delay)
            except Exception as e:
                failed += 1; consecutive_fails += 1
                err = str(e).lower()
                if "flood" in err or "too many" in err or "retry after" in err:
                    wt = 5
                    try:
                        if "retry after" in err: wt = int(''.join(filter(str.isdigit, err.split("retry after")[1]))) + 1
                    except: pass
                    await asyncio.sleep(wt); continue
                if consecutive_fails > 5: break
        
        if user_id in active_spam_tasks: del active_spam_tasks[user_id]
        if success > 0: last_spam_messages[user_id] = {"chat_id": chat_id, "message_ids": sent_message_ids, "group_name": gname}
        try: await status_msg.delete()
        except: pass
        
        result = f"""{self.bi('✅ 𝗠𝗜𝗦𝗦𝗜𝗢𝗡 𝗖𝗢𝗠𝗣𝗟𝗘𝗧𝗘𝗗 ✅')}
{self.bi('━━━━━━━━━━━━━━━━━━')}
{self.bi('📌 𝗚𝗿𝗼𝘂𝗽: ' + gname)}
...
{self.bi('👇 𝗪𝗛𝗔𝗧 𝗗𝗢 𝗬𝗢𝗨 𝗪𝗔𝗡𝗧 𝗧𝗢 𝗗𝗢 𝗡𝗘𝗫𝗧? 👇')}"""
        ...
        if success > 0: keyboard.append([InlineKeyboardButton("🗑 𝗗𝗘𝗟𝗘𝗧𝗘 𝗦𝗘𝗡𝗧 𝗠𝗦𝗚 🗑", callback_data="delete_last_spam")])
        keyboard.append([InlineKeyboardButton("🔙 𝗕𝗔𝗖𝗞 𝗧𝗢 𝗠𝗔𝗜𝗡 𝗠𝗘𝗡𝗨", callback_data="main_menu")])
        
        await context.bot.send_message(chat_id=query.message.chat_id, text=result, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=ParseMode.MARKDOWN)

async def start_telethon(): await init_telethon()

def main():
    if not BOT_TOKEN: print("❌ BOT_TOKEN not set!"); return
    loop = asyncio.new_event_loop(); asyncio.set_event_loop(loop); loop.run_until_complete(start_telethon())
    ...
    app.add_handler(MessageHandler(filters.PHOTO | filters.VIDEO | filters.AUDIO | filters.VOICE | filters.Document.ALL | filters.Sticker.ALL | filters.VIDEO_NOTE, bot.message_handler))
